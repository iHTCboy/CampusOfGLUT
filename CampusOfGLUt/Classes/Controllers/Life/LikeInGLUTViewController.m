...
#define TableID @"5519392ae4b05079796f027d"

//举报数
#define kREPORTS  30

#import "LikeInGLUTViewController.h"
...
@interface LikeInGLUTViewController ()<UITableViewDelegate,UITableViewDataSource, LifeToolBarViewDelegate,LifeTopViewDelegate, MWPhotoBrowserDelegate>
...
@implementation LikeInGLUTViewController
...
- (void)dropViewDidBeginRefreshing:(ODRefreshControl *)refreshControl
{
    ...
    [self.fetchTalkTool fetchNewLifeCircleWithTableID:TableID success:^(NSArray *fetchArray) {
        ...
        if (self.talkFrameArray.count) {
            ...
            self.ID -= 1; //旧id应该更小一个
            meassage = [NSString stringWithFormat:@"最新%ld条说说",(long)newCounts];
        }
...
-(void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"你确定要举报吗" message:@"每条信息举报超过30次后，将会暂时隐藏，审核通过后才能在显示" preferredStyle:UIAlertControllerStyleAlert];
...
{
    [self.navigationItem setRightBarButtonItem:[[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"Navi_add_normol"] style:UIBarButtonItemStylePlain target:self action:@selector(sendBtnClicked)] animated:NO];
}

...
@interface MTGGViewController ()<FocusImageViewDelegate, UITableViewDataSource,UITableViewDelegate>
...
@implementation MTGGViewController
...
- (void)loadinagMoreNews:(UIButton *)moreBtn
{
    ...
    if (self.morePage > 0)
    {
        ...
        [self.fetchNewsTool getNewsListDataWithClassName:@"媒体桂工" page:(int)self.morePage success:^(NSArray *fetchNewsArray, int nextPage)
         {
...
{
    [self.fetchNewsTool getNewsListDataWithClassName:@"媒体桂工" page:0 success:^(NSArray *fetchNewsArray, int nextPage)
     {
